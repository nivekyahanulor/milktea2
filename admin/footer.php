  ...
  <script>
	...
	<?php
	$daily_report = $mysqli->query("SELECT sum(c.total) total,c.status as order_status, d.name category_name 
								from pos_order a 
	...
	$weekly_report = $mysqli->query("select t.d,price   from 
									...
									sum(c.total) price FROM `pos_order` a 
	...
	$yearly_report = $mysqli->query("SELECT   YEAR(c.date_added) year, sum(c.total)price  from pos_order a 
									left join pos_items b on a.item_id = b.item_id 
									left join pos_checkout_order c on c.transcode =a.trans_code 
									left join pos_item_category d on d.category_id  =b.category 
  ...
  <script>
	...
	$top10   = $mysqli->query("SELECT sum(a.qty)qty ,b.item_name from pos_order a left join pos_items b on a.item_id = b.item_id");
			 while($vals = $top10->fetch_object()){
					$tops[] = array($vals->item_name, $vals->qty);
			}
    ...
    tooltip: {
        pointFormat: 'Top Products: <b>{point.y} </b>'
    },
    series: [{
        name: 'Population',
